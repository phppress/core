...
enum Message: string
{
    ...
    case UNSETTING_READ_ONLY_PROPERTY = 'Unsetting read-only property: %s::%s.';
    case UNSETTING_UNKNOWN_PROPERTY = 'Unsetting unknown property: %s::%s.';
    ...
    public function getMessage(string ...$argument): string
    {
        return match ($this) {
            ...
            self::SETTING_READ_ONLY_PROPERTY,
            self::UNSETTING_READ_ONLY_PROPERTY,
            self::UNSETTING_UNKNOWN_PROPERTY => sprintf($this->value, ...$argument),
        };
    }
}
